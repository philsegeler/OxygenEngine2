...
OE_IndexBufferUnorderedMap::OE_IndexBufferUnorderedMap(uint32_t num_of_indices) {
    ...
    this->isInit = true;
    this->data =
        unordered_map<uint32_t*, uint32_t, std::function<size_t(uint32_t*)>, std::function<bool(uint32_t*, uint32_t*)>>(
            2, lambda_hash, lambda_equals);
}
...
OE_PolygonStorage32::OE_PolygonStorage32(uint32_t num_of_vertices, uint32_t num_of_normals, uint32_t num_of_triangles,
                                         uint32_t num_of_uvmaps, uint32_t max_uv_num) {
    ...
    if ((num_of_vertices <= 65536) && (num_of_normals <= 65536) && (num_of_triangles * 3 <= 65536) && (num_of_uvmaps <= 2) &&
        (max_uv_num <= 65536)) {
...
uint32_t* OE_PolygonStorage32::addTriangleVertexIndexTuple(uint32_t* vertex_arr, uint32_t len) {
    ...
    if (len != num_of_uvs + 2) throw 1; // TODO: Proper error handling
